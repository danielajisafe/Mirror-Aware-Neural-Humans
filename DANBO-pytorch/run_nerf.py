...
def render_path(render_poses, hwf, chunk, render_kwargs,
                ...
                cams=None, subject_idxs=None, render_factor=0, root=None,
                white_bkgd=False, ret_acc=False,
                ext_scale=0.00035, base_bg=1.0, index=None,
                args=None, top_expand_ratio=1.60,
        ...
        if valid_idxs is not None:
            ...
            if bg_imgs is not None and not white_bkgd:
                if args.switch_cam: 
                    ...
                    # re-size or smoothen background with bilinear interpolation
...
def render_testset(poses, hwf, args, render_kwargs, kps=None, skts=None, cyls=None, cams=None,
    ...
    if eval_metrics:
        metrics = evaluate_metric(rgbs, gt_imgs, disps, gt_masks, valid_idxs, poses=poses,
                                  ...
                                  centers=centers, render_factor = args.render_factor, args=args)
    else:
...
def config_parser():
    ...
    parser.add_argument("--idx_map_in_ckpt", action='store_true',
                        help='is idx_map saved in saved refined checkpoint')
    ...
    parser.add_argument("--v_bright_reg", action='store_true', default=False,
                        help='add constraints to have same brightness for real and virt')
                        
    parser.add_argument("--randomize_view_dir", action='store_true', default=False,
                        help='Not to be used during training, added as a pre-definition \
                        parameter for rendering time')
    ...
    parser.add_argument("--mirr_subject", type=str, default=["S9"], nargs='+',
                        help='subject to train with on mirror data')
    ...
    parser.add_argument("--load_refined", action='store_true', default=False,
    ...
    parser.add_argument("--debug", action='store_true',
                        help='if debug use 1 GPU, instead of multiple')
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if args.opt_v_color: 
            ...
            writer.add_scalar("v_color_factor/G_v", render_kwargs_train['v_color_factor'][1].item(), i)
            writer.add_scalar("v_color_factor/B_v", render_kwargs_train['v_color_factor'][2].item(), i)
        ...
        if i % args.i_testset==0 and i > 0:
            ...
            if args.switch_cam:
                # real and virtual rendering at test time
