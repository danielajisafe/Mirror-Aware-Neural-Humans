...
class ZJUH36MDataset(ZJUMocapDataset):
    ...
    def init_meta(self):
        ...
        if self.pose_skip is not None:
            unique_kp_idxs = np.unique(self.kp_idxs)
            selected_poses = unique_kp_idxs[::self.pose_skip]
            idx_map = []
            for i, kp_idx in enumerate(self.kp_idxs):
                if kp_idx in selected_poses:
                    idx_map.append(i)
            self._idx_map = np.array(idx_map)
