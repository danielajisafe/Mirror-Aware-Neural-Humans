...
def create_raycaster(args, data_attrs, device=None):
    ...
    nerf_kwargs = {'D': args.netdepth, 'W': args.netwidth,
                   ...
                   'pts_embedder': get_pts_embedder(args, data_attrs), 
                   'pe_fn': embed_fn,
...
class RayCaster(nn.Module):
    ...
    def render_rays(self,
        ...
        # debugging
        if cyls is None:
            print(f"cyl {cyl} | v_color_factor {v_color_factor}")
            raise ValueError # still print the error
        ...
        def plot_current_pixel(dataset, image_idx, pixel_loc, type="real"):
            ...
            plt.savefig(f"checkers/imgs/pixel_loc.jpg", dpi=300, bbox_inches='tight', pad_inches = 0) 
        ...
        if use_mirr:
            ...
            near, far = self.get_near_far(rays_o=rays_o, rays_d=rays_d, cyls=cyls, near=near, far=far, skts=skts)
            ...
            n_m = normalize_batch_normal(m_normal)
            ...
            near_ref, far_ref = self.get_near_far(rays_o=v_cam_pos, rays_d=rays_ref, cyls=cyls, near=init_near_v, 
                                far=init_far_v, skts=skts)
            pts_ref, z_vals_ref = self.sample_pts(rays_o=v_cam_pos, rays_d=rays_ref, near=near_ref, far=far_ref, N_rays=N_rays, 
                                         N_samples=N_samples, perturb=perturb, lindisp=lindisp, pytest=pytest,
                                         ray_noise_std=ray_noise_std)
            del intersect_pts
            ...
            '''
            - Diff directions but same kps
            - use canonical keys for both real and virt inputs'''
            ...
            nerf_inputs_ref = self.get_nerf_inputs(
                                        pts_ref, [v_cam_pos[:, None, :], rays_ref[:, None, :]],
                                        kp_batch, skts, bones, cam_idxs=cams,
                                        subject_idxs=subject_idxs,
                                        N_uniques=N_uniques,
                                )
            ...
            '''canonical forward pass with same keys for real and virtual points/rays'''
            raw, encoded = self.network(nerf_inputs, netchunk=netchunk)
            raw_ref, encoded_ref = self.network(nerf_inputs_ref, netchunk=netchunk)
            ...
            ret_dict_ref = self.network.raw2outputs(raw_ref, z_vals=z_vals_ref, rays_d=rays_ref,
                                                raw_noise_std=raw_noise_std, pytest=pytest,
            ...
            ret_dict0_ref, encoded0_ref = None, None
            ...
            if N_importance > 0:
                ...
                if not self.single_net:
                    ...
                    # for reflected pts
                    pts_is_ref = self._merge_encodings({'pts': pts_ref}, {'pts': pts_is_ref}, sorted_idxs_ref,
                                                    N_rays, N_total_samples)['pts']
                    encoded0_ref = encoded_ref
                ...
                nerf_inputs_is_ref = self.get_nerf_inputs(
                                        pts_is_ref, [v_cam_pos[:, None, :], rays_ref[:, None, :]],
                                        kp_batch, skts, bones, cam_idxs=cams,
                                        subject_idxs=subject_idxs,
                                        N_uniques=N_uniques,
                            )
                ...
                raw_is_ref, encoded_is_ref = self.network_fine(nerf_inputs_is_ref, netchunk=netchunk)
                ...
                if self.single_net:
                    ...
                    encoded = encoded_is
                    encoded_ref = encoded_is_ref

                    raw = raw_is
                    raw_ref = raw_is_ref

                # raw2outputs {'rgb_map': rgb_map, 'disp_map': disp_map, 'acc_map': acc_map,
                ...
                ret_dict_ref = self.network_fine.raw2outputs(raw_ref, z_vals_ref, rays_ref, raw_noise_std=raw_noise_std, pytest=pytest,
                                                        encoded=encoded_is_ref, B=preproc_kwargs['density_scale'],
                                                        act_fn=preproc_kwargs['density_fn'])
            '''they are seperated here'''
            return self._collect_outputs(ret=ret_dict, ret0=ret_dict0, ret_ref=ret_dict_ref, ret0_ref=ret_dict0_ref,
                                        encoded=encoded_is, encoded0=encoded0, encoded_ref=encoded_is_ref, encoded0_ref=encoded0_ref, 
                                        use_mirr=use_mirr)
            ...
            if N_importance > 0:
                ...
                if not self.single_net:
                    # fine network needs both coarse and importance samples
                    N_total_samples = N_importance + N_samples
                    pts_is = self._merge_encodings({'pts': pts}, {'pts': pts_is}, sorted_idxs=sorted_idxs,
                                                    N_rays=N_rays, N_total_samples=N_total_samples, use_mirr=use_mirr)['pts']
                    encoded0 = encoded
                ...
                if self.single_net:
                    N_total_samples = N_importance + N_samples
                    # should be named encoded? as well the parameter passed in raw2output below as well? (naming preference)
                    ...
                    encoded = encoded_is
                    raw = raw_is
                ret_dict = self.network_fine.raw2outputs(raw, z_vals, rays_d, raw_noise_std=raw_noise_std, pytest=pytest, use_mirr=use_mirr,
                                                        encoded=encoded_is, B=preproc_kwargs['density_scale'],
                                                        act_fn=preproc_kwargs['density_fn'])

            '''they are seperated here'''
            return self._collect_outputs(ret=ret_dict, ret0=ret_dict0, encoded=encoded_is, encoded0=encoded0)
    ...
    def _merge_encodings(self, encoded, encoded_is, sorted_idxs,
                         N_rays, N_total_samples, use_mirr=False, inplace=True):
        """
    ...
    def _collect_outputs(self, ret=None, ret0=None, ret_ref=None, ret0_ref=None, encoded=None, encoded0=None, encoded_ref=None, encoded0_ref=None, use_mirr=False):
        ...
        if ret_ref is not None and use_mirr:
            ...
            collected['T_i_ref'] = ret_ref['weights']
            ...
            if ret0_ref is not None and use_mirr:
                ...
                collected['T_i0_ref'] = ret0_ref['alpha']
        ...
        # mirror
        if ret_ref is not None and use_mirr:
            if 'confd' in ret_ref and self.training:
                collected['confd_ref'] = ret_ref['confd']

                if ret0_ref is not None and use_mirr:
                        collected['confd0_ref'] = ret0_ref['confd']
        ...
        # mirror
        if ret_ref is not None and use_mirr:
            if 'j_dists' in ret_ref and self.training:
                collected['j_dists_ref'] = ret_ref['j_dists']

                if ret0_ref is not None and use_mirr:
                        collected['j_dists0_ref'] = ret0_ref['j_dists']
        ...
        # mirror
        if encoded_ref is not None and use_mirr:
            if 'gradient' in encoded_ref and self.training:
                collected['gradient_ref'] = encoded_ref['gradient']

                if encoded0_ref is not None and use_mirr:
                        collected['gradient0_ref'] = encoded0_ref['gradient']
...
class GraphCaster(RayCaster):
    ...
    def _collect_outputs(self, ret=None, ret0=None, ret_ref=None, ret0_ref=None, encoded=None, encoded0=None, encoded_ref=None, encoded0_ref=None, use_mirr=False):
        '''
        received from render_rays option above 
        '''
        # encoded0 and encoded0_ref not passsed in
        collected = super(GraphCaster, self)._collect_outputs(ret=ret, ret0=ret0, encoded=encoded, ret_ref=ret_ref, ret0_ref=ret0_ref, encoded_ref=encoded_ref, use_mirr=use_mirr)
        ...
        if 'part_invalid' in encoded and self.training:
            ...
            # mirror
            if use_mirr:
                if 'part_invalid' in encoded_ref:
                    collected['part_invalid_ref'] = encoded_ref['part_invalid']

        if 'confd' in collected: 
        ...
        if 'confd' in encoded and self.training:
            ...
            # mirror
            if use_mirr:
                if 'confd' in encoded_ref:
                    collected['confd_ref'] = encoded_ref['confd']
