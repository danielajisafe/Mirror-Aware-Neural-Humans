...
class Embedder(nn.Module):
    def __init__(self, freq_schedule=False, init_alpha=0.0, **kwargs):
        super().__init__()
        self.kwargs = kwargs
    ...
    def _embed(self, inputs, **kwargs):
        if not self.freq_schedule:
            # of shape (..., N_freqs * feat_size)
            embedded =  torch.cat([fn(inputs) for fn in self.embed_fns], -1) 
        else:
            # of shape (..., N_freqs, feat_size)
            embedded =  torch.stack([fn(inputs) for fn in self.embed_fns], dim=-2)
            pre_dim = [1 for i in embedded.shape[:-2]]
            # to shape (..., N_freqs, 1)
            w = self.get_schedule_w().reshape(*pre_dim, -1, 1)
            embedded = (w * embedded).flatten(start_dim=-2)
...
class CutoffEmbedder(Embedder):
    ...
    def __init__(self, cutoff_dist=500*0.00035,
        ...
        # TODO: poor naming ...
        # TODO: freq schedule for this is broken
...
def get_embedder(multires, i=0, input_dims=3,
                 freq_schedule=False, init_alpha=0,
