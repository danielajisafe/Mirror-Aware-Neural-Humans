...
class NeRF(nn.Module):
    def __init__(self, 
        ...
        self.randomize_view_dir = randomize_view_dir

        self.init_density_net()
        self.init_radiance_net()

        """
        if opt_r_color: # not used
    ...
    def encode_pts(self, inputs):
        '''canonical function and keys - same for real and virtual points/rays '''
    ...
    def encode_views(self, inputs, refs, encoded_pts):
        ...
        # ref https://stackoverflow.com/a/48266808/12761745
        if self.randomize_view_dir and rays_d is not None:
            radian = np.pi/2.3 # 6,4,3,2
            z_rotation = np.round(np.array([[np.cos(radian), -np.sin(radian), 0], 
                                            [np.sin(radian), np.cos(radian), 0],
                                            [0, 0, 1]]))
            rot_matrix = torch.Tensor(z_rotation.tolist())
            rays_d = rays_d @ rot_matrix
        ...
        if self.use_framecode:
            ...
            in_cam_idxs = cam_idxs.reshape(N_rays * N_samples, -1)
            framecodes = self.framecodes(in_cam_idxs)
