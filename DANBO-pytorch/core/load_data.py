...
DATASET_SKELETON_MAP = {"3dhp": SMPLSkeleton,
                        ...
                        "h36m": SMPLSkeleton,
                        "h36m_zju": SMPLSkeleton,
                        "zju": SMPLSkeleton,
                        "mixamo": SMPLSkeleton,
                        "mirror": SMPLSkeleton,
                        "perfcap": SMPLSkeleton,
                        "aist": SMPLSkeleton,
                        "neural_actor": SMPLSkeleton}
...
DATASET_CATALOG = {
    ...
    'h36m_zju': {k: f'data/h36m_zju/{k}_train.h5' for k in ['S1', 'S5', 'S6', 'S7', 'S8', 'S9', 'S11']
    },
    'aist': {k: f'data/AIST/{k}_train.h5'for k in ['d04', 'd08', 'd12']
    },
    'neural_actor': {k: f'/scratch/st-rhodin-1/users/shihyang/dataset/neuralactor/{k}/{k}_train.h5' for k in ['vlad']
    },
}
...
def load_data(args):
    ...
    if isinstance(dataset, SurrealDataset) or\
        ...
        render_data = ConcatH5Dataset([ZJUMocapDataset(DATASET_CATALOG['zju'][subject], 
                                                       split='test', subject=subject)]).get_render_data()
    elif isinstance(dataset, ConcatH5Dataset) and len(dataset.datasets) == 1 and type(dataset.datasets[0]) == ZJUH36MDataset:
        subject = args.subject[0]
        render_data = ConcatH5Dataset([ZJUH36MDataset(DATASET_CATALOG['h36m_zju'][subject], 
                                                      split='val', subject=subject)]).get_render_data()
    else:
...
def get_dataset(args):
    ...
    if len(datasets) == 1 and not args.nerf_type.startswith('mi'):
        dataset = datasets[0]
    else:
...
def get_dataset_from_catalog(args, N_samples, dataset_type, subject=None, N_nms=0,N_nms_v=0):
    ...
    if dataset_type == 'h36m':
        ...
        if args.refined_path == '':
            args.load_refined = False
        else:
            args.load_refined = True
        
        refined_paths = (args.refined_path, False)
        #'archer': ('data/mixamo/archer_refined_500ktv.tar', False)

        dataset = MirrorDataset(path, path_v, N_cams=args.N_cams, overlap_rays=args.overlap_rays,
                                train_size=args.train_size, data_size=args.data_size, idx_map=idx_map, refined_paths=refined_paths, load_refined=args.load_refined,
                                idx_map_in_ckpt=args.idx_map_in_ckpt, args=args,
                                 **shared_kwargs)

    elif dataset_type == 'zju':
        dataset = ZJUMocapDataset(path, **shared_kwargs)

    elif dataset_type =='h36m_zju':

        '''update path'''
        DATASET_CATALOG[dataset_type][subject] = args.data_path +f'/{subject}_train.h5'
        path = DATASET_CATALOG[dataset_type][subject]

        dataset = ZJUH36MDataset(path, pose_skip=args.h36m_zju_skip, 
                                 color_bg=args.h36m_zju_color_bg, **shared_kwargs)
    
    elif dataset_type == 'aist':
        dataset = AISTDataset(path, **shared_kwargs)
    elif dataset_type == 'neural_actor':
        dataset = NeuralActorDataset(path, **shared_kwargs)
    else:
