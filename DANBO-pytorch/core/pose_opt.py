...
def load_poseopt_from_state_dict(state_dict, idx_map_in_ckpt=False):
    '''
...
class PoseOptLayer(nn.Module):
    ...
    def calculate_kinematic(self, idxs, rest_pose_idxs=None):
        ...
        #mbone = self.bones[unique_indices]
...
def pose_ckpt_to_pose_data(path=None, popt_sd=None, ext_scale=0.001, legacy=False, skel_type=SMPLSkeleton, idx_map_in_ckpt=False):
    # TODO: ext_scale is hard-coded for now. Fix it later

    if popt_sd is None:
        popt_sd = torch.load(path)['poseopt_layer_state_dict']

    poseopt = load_poseopt_from_state_dict({'poseopt_layer_state_dict': popt_sd}, idx_map_in_ckpt=idx_map_in_ckpt)
    ...
    with torch.no_grad():
        ...
        if idx_map_in_ckpt:
            import_idx_map = poseopt.idx_map.cpu().numpy().astype(np.int) # ()[0].cpu().numpy()
        else:

            idx_map_file = "/".join(path.split("/")[:-1]) + "/idx_map.npy"
            if os.path.isfile(idx_map_file):
                import_idx_map = np.load(idx_map_file).astype(np.int)
            else:
                import_idx_map = None
    ...
    # start with current data template
    kp3d_temp = kp3d.copy()
    bones_temp = bones.copy()
    skts_temp = skts.copy()
    cyls_temp = cyls.copy()
    pelvis_temp = pelvis.copy()

    if import_idx_map is not None:
        # do groundwork here
        idx_map_indices = import_idx_map.tolist() # set(list(range(2000))) - set(idx_map.tolist())
        n = len(idx_map_indices)
        for i in range(n):
            val = idx_map_indices[i]

            kp3d_temp[val] = kp3d[i]
            bones_temp[val] = bones[i]
            skts_temp[val] = skts[i]
            cyls_temp[val] = cyls[i]
            pelvis_temp[val] = pelvis[i]

        # updated with idxs that were refined from import
        kp3d = kp3d_temp.copy()
        bones = bones_temp.copy()
        skts = skts_temp.copy()
        cyls = cyls_temp.copy()
        pelvis = pelvis_temp.copy()
    
    return kp3d, bones, skts, cyls, rest_pose, pelvis, import_idx_map
