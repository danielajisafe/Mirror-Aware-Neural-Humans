...
def MPJPE(ref_pose, fit_pose) -> float:
    ...
    # Calculates Euclidean or Distance. Sqrt((x2-x1)2 + (y2-y1)2 + ...); 
    difference = ref_pose - fit_pose
    square_difference= torch.square(difference)
    dist = torch.sqrt(torch.sum(square_difference, dim=-1)) # sum over the joint (x,y,z) 
